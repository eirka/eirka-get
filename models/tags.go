...
// TagsSearchModel holds the parameters from the request
...
// Get will look in the database and return the matches for the particular tag
func (i *TagSearchModel) Get() (err error) {
	...
	// format the searchterm
	searchterm := fmt.Sprintf("%%%s%%", i.Term)

	tags := []Tags{}

	rows, err := db.Query(`select tag_id,tag_name,tagtype_id
	FROM (select count(image_id) as count,ib_id,tags.tag_id,tag_name,tagtype_id
	...
	for rows.Next() {
		...
		tag := Tags{}
		// Scan rows and place column into struct
		err := rows.Scan(&tag.Id, &tag.Tag, &tag.Type)
		if err != nil {
			return err
		}
