...
func TestRandomModelGet(t *testing.T) {
	...
	t.Run("Valid request with full data", func(t *testing.T) {
		// Mock random image query
		imageRows := sqlmock.NewRows([]string{
			"image_id", "thread_id", "post_num", "post_id", "image_file", "image_orig_height", "image_orig_width",
		}).AddRow(5, 10, 3, 15, "random.jpg", 800, 600)

		mock.ExpectQuery(`SELECT image_id, posts.thread_id, posts.post_num, posts.post_id, image_file, image_orig_height, image_orig_width FROM images.+ORDER BY RAND\(\) LIMIT 1`).
			WithArgs(1).
		...
		assert.Equal(t, "random.jpg", model.Result.Body.File, "Filename should match")
	...
	// Test case 2: Valid request with no prev/next images and no tags
	t.Run("Valid request with no prev/next and no tags", func(t *testing.T) {
		// Mock random image query
		imageRows := sqlmock.NewRows([]string{
			"image_id", "thread_id", "post_num", "post_id", "image_file", "image_orig_height", "image_orig_width",
		}).AddRow(5, 10, 3, 15, "random.jpg", 800, 600)
	...
	// Test case 3: Empty imageboard parameter
	t.Run("Empty imageboard parameter", func(t *testing.T) {
		model := RandomModel{
			Ib: 0,
		}

		err := model.Get()
		assert.Error(t, err, "Should return error for empty imageboard parameter")
		assert.Equal(t, e.ErrNotFound, err, "Should return ErrNotFound")
	})

	// Test case 4: No random image found
	t.Run("No random image found", func(t *testing.T) {
		mock.ExpectQuery(`SELECT image_id, posts.thread_id, posts.post_num, posts.post_id, image_file, image_orig_height, image_orig_width FROM images.+ORDER BY RAND\(\) LIMIT 1`).
			WithArgs(1).
		...
		err := model.Get()
		assert.Error(t, err, "Should return error for no random image found")
		assert.Equal(t, e.ErrNotFound, err, "Should return ErrNotFound")
	})
	...
	// Test case 6: Error in random image query
	t.Run("Error in random image query", func(t *testing.T) {
		...
		err = model.Get()
		assert.Error(t, err, "Should return error for random image query failure")
